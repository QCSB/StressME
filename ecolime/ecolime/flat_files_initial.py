...
def get_aggregation_popensity_df():
    ...
    df = pandas.read_csv(fixpath('aggregation_propensity.csv'))
    return df.set_index('gene')

def get_ML_keff_df():
    """returns the machine-learned keffs from David Heckmann"""
    df = pandas.read_csv(fixpath('kappmax_kcat_in_vitro_rf_dl_iJO.csv'), index_col=0)
    df.drop(['experim_kappmax'], axis=1, inplace=True)
    df.clip_lower(2.2e-5)
    return df

def get_ML_keff_id_mapping_dict():
    """ returns the mapping"""
    df = pandas.read_csv(fixpath('david_m_id_to_me_id.csv'),index_col=0).fillna(0)
    david_id_to_me = df.to_dict()['me_id']
    return david_id_to_me
