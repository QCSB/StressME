...
from cobrame.io.json import save_json_me_model
from cobrame.io.json import validate
from cobrame.io.dict import me_model_to_dict
from cobrame.io.json import get_schema

from cobra import Reaction
from cobrame import MEReaction


me_schema = get_schema()
...
def return_me_model():
    ...
    me.ngam = 13.58
    me.gam = 19.49
    me.unmodeled_protein_fraction = .10
    ...
    met=me.metabolites.get_by_id('CPLX0-7824_mod_1:4fe4s')
    met.formula='C1190H1800N314O352S18Fe4'
    met=me.metabolites.get_by_id('CPLX0-7617_mod_1:4fe4s')
    met.formula='C1056H1624N270O356S16Fe4'
    met=me.metabolites.get_by_id('IscA_tetra_mod_1:4fe4s')
    met.formula='C2040H3168N540O644S24Fe4'
    
    met=me.metabolites.get_by_id('CPLX0-7824_mod_1:2fe2s')
    met.formula='C1190H1800N314O352S16Fe2'
    met=me.metabolites.get_by_id('CPLX0-7617_mod_1:2fe2s')
    met.formula='C1056H1624N270O356S14Fe2'
    met=me.metabolites.get_by_id('IscA_tetra_mod_1:2fe2s')
    met.formula='C2040H3168N540O644S22Fe2' 
    ...
    me.ngam = 13.58
    me.gam = 19.49
    me.unmodeled_protein_fraction = .40
    ...
    Lon_demand = Reaction('Lon_demand')
    me.add_reaction(Lon_demand)
    Lon_demand.add_metabolites({'Lon':-1.})    
    Lon_demand.upper_bond = 1000.
    Lon_demand.lower_bond = 0.
...
#    n_genes, n_genes * 100. / (1678)))
    ...
    # 1. None compartments not allowed
    for d in me.metabolites:
      if d.compartment is None:
        d.compartment = 'xx'
        
# 2. 'EG10845' is not of type 'number'. The unique_name shouldn't be there.
    for d in me.process_data:
      if hasattr(d, 'keq_folding'):
        if 'unique_name' in d.keq_folding:
            d.keq_folding.pop('unique_name')

# 3. Same error as above but for k_folding
    for d in me.process_data:
      if hasattr(d, 'k_folding'):
        if 'unique_name' in d.k_folding:
            d.k_folding.pop('unique_name')            
            
# 4. '419.0' is not of type 'number': protein_length stored as str in keq_folding
    for d in me.process_data:
      if hasattr(d, 'keq_folding'):
        if 'protein_length' in d.keq_folding:
            if isinstance(d.keq_folding['protein_length'], str):
                d.keq_folding['protein_length'] = float(d.keq_folding['protein_length'])

# 5. Same error as above but for protein_length in k_folding
    for d in me.process_data:
      if hasattr(d, 'k_folding'):
        if 'protein_length' in d.k_folding:
            if isinstance(d.k_folding['protein_length'], str):
                d.k_folding['protein_length'] = float(d.k_folding['protein_length'])
    
# 6. k_folding['To'] being str
    for d in me.process_data:
      if hasattr(d, 'k_folding'):
        if 'To' in d.k_folding:
            if isinstance(d.k_folding['To'], str):
                d.k_folding['To'] = float(d.k_folding['To'])

# 7. MetabolicReaction is a required property on instance {'Reaction': {}}. Lon_demand wasn't assigned one of the required reaction_type

    me.reactions.Lon_demand = MEReaction(me.reactions.Lon_demand)
    ind = me.reactions._dict['Lon_demand']
    me.reactions[ind] = MEReaction(me.reactions[ind])
    
    me_dict = me_model_to_dict(me)
    badrxns = []
    for r in me_dict['reactions']:
      if 'Reaction' in r['reaction_type']:        
        badrxns.append(r)
        
    me_dict = me_model_to_dict(me)
    validate(me_dict, me_schema)
    
# 8. adjust keff for wildtype
...
if __name__ == '__main__':
    ...
    me = return_me_model()
    with open("%s/EcoliME_heatevolved.pickle" % here, "wb") as outfile:
        pickle.dump(me, outfile)
    with open("%s/me_models/EcoliME_heatevolved.pickle" % here, "wb") as outfile:
        pickle.dump(me, outfile)

    save_json_me_model(me, "%s/me_models/EcoliME_heatevolved.json" % here)
