...
def get_string_dict(rxn_dict):
    ...
    rxn_string_dict = {}
    for met, stoich in rxn_dict.items():
...
def get_lipid_composition(me_model, fatty_acid_comp_df, cur_strain_name):
    ...
    cur_index = 0
    for cur_lipid, cur_lipid_mol_fraction in phospholipid_mol_fraction.items():
        for cur_fa, cur_fa_mol_fraction in fatty_acid_fraction.items():
