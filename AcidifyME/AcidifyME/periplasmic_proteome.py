...
def get_protein_net_charge_table():
	...
	for cur_gene in proteins_w_charge_calc:
	    ...
	    for line in cur_file:
	        ...
	        protein_net_charge_df[cur_gene] = list(map(float, line.split()[1:]))
	    cur_file.close()
...
def calc_e_folding_energy(b_num, pH, temperatureK, IS=0.25):
    diele_water = 80.4; R = 8.314
    l_b = 1.39/10**4/diele_water/R/temperatureK #in meter
    ...
    dG_e = R*temperatureK*(np.power(protein_charge, 2) * l_b / (2 * protein_Rg * (1 + k * protein_Rg)) - \
                np.power(peptide_charge, 2) * l_b / (2 * peptide_Rg * (1 + k * peptide_Rg)))
    return dG_e/1000 #unit in kJ/mol

def calc_tot_folding_energy_at_pH(b_num, pH, temperatureK):
    dG_folding_pH7 = calc_tot_folding_energy_from_b_num(temperatureK, b_num)
    dGe_folding_pH7 = calc_e_folding_energy(b_num, 7.0, temperatureK)
    dGe_folding_pH = calc_e_folding_energy(b_num, float(pH), temperatureK)
...
def add_periplasm_protein_folding_reaction(me_model, pH, temperatureK):
    ...
    for cur_protein in me_model.metabolites.query('Periplasm'):
        ...
        if cur_bnum not in protein_net_charge_df.columns:
            ...
            cur_folding_energy = calc_tot_folding_energy_at_pH(cur_bnum, pH, temperatureK) * 1000 #to make the unit J/mol
        keq_folding = 1.0/np.exp(cur_folding_energy/(-R*temperatureK)) #unfolded/folded
...
def add_protein_stability_constraint(me_model, pH, temperatureK, constrain_periplasm_biomass=True):
    add_periplasm_protein_folded(me_model)
    add_periplasm_protein_folding_reaction(me_model, pH, temperatureK)
    ...
    if constrain_periplasm_biomass:
        add_periplasm_biomass_constraint(me_model, 0.50, 0.10, 0.40)