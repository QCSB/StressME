...
class MEModel(Model):
    ...
    def prune(self, skip=None):
        ...
        """
        if not skip:
            skip = ['protein_b0605','protein_b0606','protein_b2962','protein_b4209','protein_b3662','protein_b0812','protein_b3961','protein_b4062','protein_b4063','protein_b3509']

        complex_data_list = [i.id for i in self.complex_data
                             if i.id not in skip]
        ###################### add on 2021/02/17
        print("complex_data_list",complex_data_list)
        ######################
        ...
        ######################
        #        print("self.process_data.get_by_id(c_d)", self.process_data.get_by_id(c_d))
        ######################
